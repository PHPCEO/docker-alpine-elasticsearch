...
MAINTAINER Blue Apron Devops <devops@blueapron.com>
...
RUN apk update && apk add curl openjdk8-jre && \
  ...
  curl -o elasticsearch-2.3.5.tar.gz -sSL https://download.elasticsearch.org/elasticsearch/release/org/elasticsearch/distribution/tar/elasticsearch/2.3.5/elasticsearch-2.3.5.tar.gz && \
  tar -xzf elasticsearch-2.3.5.tar.gz && \
  rm elasticsearch-2.3.5.tar.gz && \
  mv elasticsearch-2.3.5 /usr/share/elasticsearch && \
